...
import com.medical.common.tools.TmtClientTools;
...
public class DialogueApi {
    ...
    @Resource
    private TmtClientTools tmtClientTools;
    @Resource
    ...
    public R send(@RequestBody @Valid OnlineConsultationDialogueSend req) {
        ...
        String translationText = tmtClientTools.translationText("zh", dialogue.getContent(), "ja");
        if(!dialogue.getContent().equals(translationText)){
            String content = dialogue.getContent() + " 翻訳する: " + translationText;
            dialogue.setContent(content);
        }
        dialogueService.sendDialogue(dialogue, false, playerTokenResp.getId(), playerTokenResp.getUsername(),null, null,2,1,"处方药在线挨训");
        return R.ok(null);
    }
    ...
    public R sendPrescriptionMessage(@RequestBody @Valid OnlinePrescriptionDialogueSend req) {
        ...
        dialogueService.sendDialogue(dialogue, false, playerTokenResp.getId(), playerTokenResp.getUsername(),null, null,1,6,"重大疾病在线咨询");
        return R.ok(null);
    }
