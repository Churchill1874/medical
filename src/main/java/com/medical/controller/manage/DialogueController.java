...
public class DialogueController {
    ...
    public R send(@RequestBody @Valid OnlineConsultationDialogueSend req) {
        ...
        String translationText = tmtClientTools.translationText("ja", dialogue.getContent(), "zh");
        if(!dialogue.getContent().equals(translationText)){
            String content = dialogue.getContent() + " 翻译: " + translationText;
            dialogue.setContent(content);
        }
        dialogueService.sendDialogue(
                dialogue,
                true,
                admin.getId(),
                admin.getName(),
                onlineConsultation.getUserId(),
                onlineConsultation.getRealName(),
                2,
                6,
                "发送在线咨询重大疾病对话");
        return R.ok(null);
    ...
    public R sendPrescriptionMessage(@RequestBody @Valid OnlinePrescriptionDialogueSend req) {
        ...
        dialogueService.sendDialogue(
                ...
                admin.getId(),
                admin.getName(),
                onlinePrescription.getUserId(),
                onlinePrescription.getRealName(),
                1,
                1,
                "发送处方药咨询消息");
        return R.ok(null);
