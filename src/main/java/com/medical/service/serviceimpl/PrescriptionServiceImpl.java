...
public class PrescriptionServiceImpl extends ServiceImpl<PrescriptionMapper, Prescription> implements PrescriptionService {
    ...
    public void addPrescription(Prescription prescription) {
        ...
        String content = "开处方药下订单了";
        newMessageService.addNewMessage(2, null, content , prescription.getId(), prescription.getUserId(), playerTokenResp.getUsername());
...
/*        Admin admin = adminService.findByAccount(InitConfig.SUPER_ADMIN_ACCOUNT);
    ...
    public Prescription findById(Long id, Long userId) {
        Prescription prescription = getById(id);
        newMessageService.deleteNewMessage(Lists.newArrayList(2), null,prescription.getId(),userId);
        return prescription;
    }
